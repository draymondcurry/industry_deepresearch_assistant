...
行业研究作为金融领域的重要课题，在实际应用层面中存在严重的信息不对称、数据分析能力不足、决策成本高等问题。本项目旨在基于海量银行内部研究报告与经济数据，以及实时抓取的外部政策文件、新闻资讯搭建了一个具有复杂任务规划、反思与执行能力的AI Agent系统，结合长短期记忆机制与工程优化，为客户提供普惠、专业、高效的商业决策支持。
