...
export default function Index() {
  ...
  // 记录面板是否曾经被打开过（一旦打开过，收起后就永远显示悬浮按钮）
  const [researchPanelEverOpened, setResearchPanelEverOpened] = useState(false)
        ...
        try {
          ...
          if (target.type === ChatType.Deepsearch) {
            ...
            if (json.type === 'research_start') {
              ...
              setResearchPanelEverOpened(true)
            }
  ...
  // 悬浮按钮显示条件：面板曾经被打开过，且当前是关闭状态
  const showFloatingBtn = researchPanelEverOpened && !researchPanelOpen
  ...
  return (
    <ComPageLayout
      ...
      rightPanelOpen={researchPanelOpen}
      onRightPanelClose={() => setResearchPanelOpen(false)}
      onRightPanelReopen={() => setResearchPanelOpen(true)}
